 ...
 * Runs before LLM planning so validation phases type bad passport/dates/etc.
 */

"use strict";

const { resolveTestdata } = require("./testAgent");
const { isAhcGroup } = require("./ahcGroups");

function isEdgeSpec(spec) {
...
function phaseWantsValidation(spec, phase) {
...
function botAsks(last, patterns) {
  const t = String(last || "").toLowerCase();
  return patterns.some((p) => p.test(t));
}

function botRejectedInput(last) {
...
function phaseExpectsInvalidNow(phase, attempts) {
  ...
  if (attempts === 1) {
    if (/invalid|reject|wrong|past|future|over_|text_instead|digits_only|end_before|name_with/.test(pid)) {
      return true;
    }
    if (/enter invalid|invalid input|all-numeric|past date|future year|over 100|non-numeric|before the start/i.test(desc)) {
      return true;
    }
    if (/reject|not valid|invalid|error message|must be|between \d/i.test(crit)) {
      return true;
    }
  }
  return false;
...
/**
 * @returns {{ action: string, value: string, rationale: string } | null}
 */
function suggestEdgeCasePlan({ phase, spec, lastBotText, attempts = 1 }) {
  ...
  // After bot rejected input, prefer valid testdata for the same field
  if (afterReject) {
    ...
    if (botAsks(last, [/percentage|between 1 and 12/i])) {
      const v = pick(td, "validPercentage", "percentage", "valid_percentage") || "10";
      return { action: "type", value: v, rationale: `Valid VPF % after rejection (${v})` };
    }
  ...
  if (spec.groupId === "uk_visa") {
    if (
      (pid.includes("invalid_passport") || /all-numeric passport|123456789/.test(desc)) &&
      botAsks(last, [/passport number/i, /passport/i])
    ) {
    ...
    if (
      (pid.includes("name_with_numbers") || pid.includes("alphanumeric_name")) &&
      botAsks(last, [/full name/i, /name.*passport/i, /applicant.*name/i])
    ) {
      const v =
        pick(td, "invalidName", "invalid_name") ||
        (pid.includes("john") ? "John123Smith" : "Geetika123");
      return { action: "type", value: v, rationale: `Edge: invalid name with numbers (${v})` };
    }
    if (
      (pid.includes("past_start") || desc.includes("past date")) &&
      botAsks(last, [/start date/i, /travel/i, /planning to travel/i, /date/i])
    ) {
    ...
    if (
      (pid.includes("end_before_start") || desc.includes("before the start")) &&
      botAsks(last, [/return date/i, /end date/i, /return/i])
    ) {
    ...
    if (
      (pid.includes("complete") || pid.includes("valid")) &&
      botAsks(last, [/start date/i, /travel/i]) &&
      pick(td, "validStartDate", "startDate")
    ) {
      const v = pick(td, "validStartDate", "startDate");
      return { action: "type", value: v, rationale: `Valid start date (${v})` };
    }
    if (
      (pid.includes("complete") || pid.includes("valid_end")) &&
      botAsks(last, [/return date/i, /return/i]) &&
      pick(td, "validEndDate", "endDate")
    ) {
      const v = pick(td, "validEndDate", "endDate");
  ...
  if (isAhcGroup(spec.groupId)) {
    if (
      (pid.includes("invalid_mobile") || pid.includes("phone")) &&
      botAsks(last, [/mobile|phone|10-digit|digit number/i])
    ) {
      const v = pick(td, "invalidMobile", "invalid_mobile", "badMobile") || "12345";
      return { action: "type", value: v, rationale: `Edge: invalid mobile (${v})` };
    }
    if (
      (pid.includes("past_date") || pid.includes("invalid_calendar") || pid.includes("sunday")) &&
      botAsks(last, [/appointment date/i, /preferred date/i, /date/i])
    ) {
      const v =
        pick(td, "pastDate", "invalidDate", "sundayDate", "tooSoonDate", "_pastDate") ||
        "31 Feb 2026";
    ...
    if (pid.includes("spouse") && pid.includes("dob") && botAsks(last, [/date of birth/i, /dob/i, /birth/i])) {
      const v =
        pick(td, "invalidSpouseDob", "futureSpouseDob", "underageSpouseDob") ||
        pick(td, "todaySpouseDob");
  ...
  // ── Appraisal letter (future year, irregular year) ──────────────────────────
  if (spec.groupId === "appraisal_letter") {
    if (
      (pid.includes("future") || pid.includes("invalid_year") || /2030|future year/.test(desc)) &&
      botAsks(last, [/4-digit year|enter the year|year between/i])
    ) {
  ...
  // ── Car purchase (invalid mobile before valid) ────────────────────────────
  if (spec.groupId === "car_purchase") {
    if (
      wantsInvalid &&
      (pid.includes("invalid_mobile") || /invalid mobile|wrong mobile/.test(desc + pid)) &&
      botAsks(last, [/mobile|10-digit|phone/i])
    ) {
      const v =
        pick(td, "invalidMobile", "invalid_mobile", "invalidMobile12", "invalidMobile11", "invalidMobileShort") ||
        "932101243221";
      return { action: "type", value: v, rationale: `Edge: invalid mobile (${v})` };
    }
    if (
      wantsInvalid &&
      pid.includes("invalid") &&
      botAsks(last, [/on road price|on-road price|price/i])
    ) {
      const v = pick(td, "invalidPrice", "invalid_price") || "abc";
      return { action: "type", value: v, rationale: `Edge: invalid price (${v})` };
    }
    if (
      (pid.includes("valid_mobile") || /valid.*mobile|continue.*price/i.test(desc + pid)) &&
      botAsks(last, [/on[- ]?road price|price of the car/i])
    ) {
      const v = pick(td, "onRoadPrice", "on_road_price");
      if (v) {
        return { action: "type", value: v, rationale: `Continue car flow: on-road price (${v})` };
      }
    }
  }
  ...
  if (spec.groupId === "motorcycle_purchase") {
    if (
      wantsInvalid &&
      (pid.includes("invalid_mobile") || /invalid mobile|not a valid 10 digit/.test(desc + pid)) &&
      botAsks(last, [/mobile|10-digit|phone|digit number/i])
    ) {
      const v =
        pick(td, "invalidMobile", "invalid_mobile", "invalidMobile11") || "94621376542";
      return { action: "type", value: v, rationale: `Edge: invalid mobile (${v})` };
    }
    if (
      wantsInvalid &&
      (pid.includes("invalid_city") || pid.includes("wrong_city") || /not a city|not valid city|try another location/i.test(desc)) &&
      botAsks(last, [/city/i])
    ) {
    ...
    if (
      wantsInvalid &&
      pid.includes("city_not_in_state") &&
      botAsks(last, [/city/i, /haryana|state/i])
    ) {
  ...
  // ── Employment letter (casual / Hinglish intents) ─────────────────────────
  ...
  if (spec.groupId === "voluntary_provident_fund") {
    if (
      (pid.includes("invalid_percentage") || pid.includes("over_100") || pid.includes("text_instead")) &&
      botAsks(last, [/percentage|between 1 and 12|valid percentage/i])
    ) {
      const v =
        pick(td, "invalidPercentage", "invalid_percentage") ||
        (pid.includes("over") ? "150" : pid.includes("text") ? "five percent" : "0");
